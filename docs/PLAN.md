...
| **导航高亮** | 顶部一级导航按当前路径高亮；子页面归属其所属一级功能（产品详情页高亮"搜索"）。**2026-09-24 CR8-5 复核：主人明确要求保留此归属**（他需要一级区分），即使从首页热点进入详情页也仍高亮"搜索" | `app/components/Nav.tsx:13`（`aria-current="page"`） |
| **~~面包屑~~**（2026-09-24 起作废，CR8-5） | **不再显示面包屑**。原约定「首页 / 一级功能 / 当前项」要求每个页面有固定父级，但产品详情页没有（一只股票不隶属某个一级功能），实现只能把中间那级**硬编码成"搜索"**，于是从任何入口进来都谎报来路（CR8-5）。二级页面改用下三行表达来路与可达性 | 原用于搜索页与产品详情页；`app/components/Breadcrumbs.tsx` 随 CR8-5 删除 |
| **返回入口**（2026-09-24 新增，CR8-6） | 二级及以下页面在标题区提供「← 返回」，语义是**返回来源页**而非"上一级"：用 `router.back()` 以保住搜索词与列表滚动位置（与上「搜索现场保留」同向，是其消费端）；无历史记录时（直接敲 URL / 新标签 / 刷新）退化为跳「首页」 | ⬜ 待实施（CR8-6） |
| **一级导航固定**（2026-09-24 新增，CR8-4） | 顶部导航 `sticky`，长页面滚动时一级入口始终可达；页内锚点跳转必须带 `scroll-margin` 补偿，避免目标标题被固定头部压住 | ⬜ 待实施（CR8-4） |
| **来源可追溯**（2026-09-24 新增，CR8-3） | 外部来源链接必须**带标题呈现**（列表内截断显示 + 悬浮显示完整标题），不允许渲染成 N 个无差别的"原文"；该条目无来源链接时整行不渲染（不显示空标签） | ⬜ 待实施（CR8-3） |
...
- P3 增补：海外源不可达时自动切换国内新闻源且界面有显式降级标注（R12）；服务重启后缺失 digest 自动补跑
  - **⚠️ 2026-09-24 CR8-1 修订本条口径**：新闻源改为**国内为主**、Tavily 降为回退；降级标注**按分类在页头显示一次**，板块名映射未命中不再对用户显示。原"海外源优先 + 逐卡显式标注"的验收表述作废（见 R12 与「CR8 界面语义批次决策记录」）
...
### CR8 界面语义批次决策记录（2026-09-24，主人本地实跑点击驱动）

> 主人实操首页热点卡片流提出 6 项观感问题，逐条回代码取证后判定：**除 CR8-7 外，六项都不是"功能没实现"，而是"实现了但界面对它的表达是错的"**。
> 发现原文、行号锚点与实测证据只在 [CODE-REVIEW.md · CR8](CODE-REVIEW.md)，本表只记结论与理由；实施状态只在 [FIX-LEDGER.md](FIX-LEDGER.md) 看板维护。

| 编号 | 决策 | 理由（一句话） |
|---|---|---|
| **CR8-1** | 新闻源序**反转为国内为主**（可多源、按数据丰富度排序回退）；`degraded` 拆为分类 `reasons[]`，② LLM 退化只在页头显示一次小字，③ 板块名映射未命中**不再显示** | 本机 Tavily 时通时不通，降级是常态而非异常；旧口径把三件事 OR 成一个布尔并逐卡复制，噪声淹没真信号 |
| **CR8-2** | 「深度解读」**改名「去分析 <股票名>」**，不做真解读 | 它本来就只是个链接，不触发分析也不带热点上下文；改名即诚实，真做需另立需求（接口加上下文参数 + 研报缓存键） |
| **CR8-3** | 「原文」改为**独立成行的「相关文章」**，带截断标题 + 悬浮全文 | 标题在 `pipeline.py:553` 被丢弃，前端无从渲染；同时要求国内主源必须选**有链接**的东财快讯而非财联社电报 |
| **CR8-4** | 顶部导航改为 `sticky` 固定 | 长卡片流下滚到底就失去一级入口；配套必须给 `#research` 锚点加 `scroll-mt` |
| **CR8-5** | **撤销面包屑约定**（搜索页 + 详情页都删，`Breadcrumbs.tsx` 随之删除）；**Nav 的 `/product`→「搜索」高亮按主人要求保留** | 详情页没有固定父级，面包屑只能硬编码"搜索"，构成谎报；主人明确要保留高亮以维持一级区分 |
| **CR8-6** | 详情页加「← 返回」，用 `router.back()`，无历史时退化为「首页」链接 | 删面包屑后它是唯一回指入口；`back()` 才能保住搜索词，对齐「搜索现场保留」 |
| **CR8-7** | ⏳ **未拍板**，登记为 [FIX-LEDGER.md](FIX-LEDGER.md) 的 `OPT-2` | 新闻层多源合并属取数模型变更（R15 现为 failover 语义），且已量化出会加剧 eastmoney 令牌桶争抢，须先实测再设计 |

**本批不改动项（明确保留）**：`Nav.tsx:13` 的 `/product` 高亮、蓝色板块标签跳 `/search?q=<板块>` 的设计（板块→成分产品列表）。
