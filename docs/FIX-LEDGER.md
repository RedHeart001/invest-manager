...
> **速览**：想知道"现在该做什么"看这里。**当前未闭环 17 项** = **CR7 的 11 项**（CR7-1~5 已修（6 项闭环：1/2/3/4/5/14），CR7-6 部分闭环；剩余 = 批次 C 的 7/8/9/10 + C5 主路由 + 批次 D 的 11/12/13）+ **CR8 的 6 项**（2026-09-24 界面语义审查，方案已全部拍板、代码未动）+ G7 鉴权 + C31 Docker 验证（看板列 25 行 = 17 未闭环 + 7 已修/闭环 + 1 部分闭环 + 3 别名行；`G3`/`G6 消费侧`/`C31 关联` 为别名，状态指回主行、不在本行复述）；**CR8 侧 2 项待定**（批次范围 + OPT-2 是否立项，见下「待你拍板」）。优化候选 OPT-1 已落地（2026-09-22 技能路由改 `load_skill`）；OPT-2（新闻层多源合并）2026-09-24 登记，**未查证完不升级为 PLAN 设计**。发现原文在 [CODE-REVIEW.md](CODE-REVIEW.md)，本文件不管"发现了什么"，只管"修没修、怎么验、下一步做什么"。
...
| **CR7-6** | GET 取数端点无 code 校验，可被耗尽东财额度 | P2 | 🟡 部分闭环（2026-09-25） | 新增 `/api/quote/verify` 已带 CODE_SET+type 白名单；`/api/kline`、`/api/quote` 主路由待批次 C（C5） |
...
## CR7 · 修复计划（✅ 批次 A+B+D4-①②④ 已完成（2026-09-25，`13be2cf`+`d4c9703`）；批次 C 留下一轮）
...
3. **批次 C**（留下一轮）：**C0 实测 → C1/C2/C3/C5 → C4**（额度与调度一次改齐，避免反复试）；C0 实测另择空闲时段单独跑；
4. **D1**（下轮紧随：P1 两项目前都只有新写的测试在守，回归面要成对）；D4-② 已闭环。
