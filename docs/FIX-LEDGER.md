...
> **2026-09-25 校验插曲（对批次 C 的输入）**：本次 verify-all 串行跑 7 套件把东财源族令牌桶打满并触发熔断（p2 独占 121s），叠加"分钟线无备源"造成 p2 唯一失败。这实证了 CR7-7（源族额度敏感）的日常风险——**批次 C 做 C0 实测与 C1 串行化时，应把「分钟线无备源」一并纳入评估**（是否给腾讯补分钟线，或 1D 档位默认走日 K 缓存）。
