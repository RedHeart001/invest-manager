...
| **CR3** | 09-14 | `d6ac165` | **无任何文档记载** | ？（8 项） | ⏳ 待重建 |
...
| **CR6** | 09-18~20 | **未提交** | "第一轮·全项目"（**误名**） | `CR-01..22` + `G1–G7` + `V1–V3` | 已处置；遗留 G3/G6/G7 |
| **CR7** | 09-20 | **未提交** | "第二轮·全项目"（**误名**） | `CR7-1..14` | **全部未修复** |
...
**CR3 的重建线索**：`d6ac165`（2026-09-14）"fix: 第三轮 code review——8 项缺陷全修（含 1 项回归 + 1 项不完整修复）"。该轮在 PLAN.md / Progress.md / code-review.md 中**均无记载**，需从 `git show d6ac165` 还原。注意其"1 项不完整修复"很可能就是后来 CR4/CR5 反复出现的"修复只做了一半"模式的源头（C11→CR4-4、CR4-6→CR5-3、CR7-1）。

---

# CR7 · 第二轮·全项目（2026-09-20，当前轮）
...
> **状态**：⏳ **全部未修复**。修复计划与状态见 [FIX-LEDGER.md](FIX-LEDGER.md)。
...
> **结果**：✅ 已全部处置完毕（22 项代码风险 + 7 项需求缺口），并经双服务全量集成验收。
> **本节的逐条 prose 已归档**，结论表保留在此；完整原文见 [history/2026-09-18-cr6-批次ABCD执行明细.md](history/2026-09-18-cr6-批次ABCD执行明细.md)。
...
> ⚠️ **本表 G3/G6 的 ✅ 已被 CR7 修正**：CR7-3 指出 G3 端点**零调用方**（已复核确认）、CR7-4 指出 G6 消费侧三处断链。最终状态见 [FIX-LEDGER.md](FIX-LEDGER.md) 看板。
...
- **修复**：`web/lib/kline.ts` 改传 `dayStart(c.date).getTime()`；一次性数据修复脚本 `web/scripts/fix-kline-date.mjs`。
...
1. **test-p5 `sources.length >= 2`**：`meta.sources` 按来源名去重，行情/K线/新闻同源时塌缩为 1 项 → 误报。改为断言新增的 `meta.dimensions`。
2. **test-p5 对"当日已有研报"的环境依赖**：跨天（北京时间 00:00 后）首次运行必失败。改为按当日实际状态分流断言。
3. **test_cr6_pipeline 浮点边界断言**：`deadline = t0 + 300` 与断言处 `monotonic()` 存在微秒差，`<= 300` 过严 → 放宽为 `<= 300.5`。

### 本地环境验证

- **G6 港股连通性**：✅ **用户本地实测通过**（`POST /api/sync?type=hk` 正常）。排障过程与根因见上「V3」。
...
| **CR3**（09-14） | ⏳ **无文档记载**。仅知提交 `d6ac165`："第三轮 code review——8 项缺陷全修（含 1 项回归 + 1 项不完整修复）"。 | 待从 `git show d6ac165` 重建，见 [history/2026-09-14-cr3-第三轮重建.md](history/2026-09-14-cr3-第三轮重建.md) |
