...
- **开发已获批准按阶段推进**（2026-09-12 起解除"仅规划"限制）：用户逐阶段显式批准（"开始 P3"/"开始 P5"等），执行时严格遵从 PLAN.md，进度记录 Progress.md。截至 **2026-09-13 15:50 状态确认**：**P0–P7 全部完成 + 全项目代码审查 C1–C15 + O 系列优化 12 项全部落地**。唯一未闭环：L2 web 镜像瘦身后的镜像重建验证（阻塞于 Docker Desktop 未运行，代码已就绪）。
...
- **项目未初始化 git**：无版本追溯能力（重要风险，建议 `git init` + 首次提交基线）。
- 东财数字货币行情源可用性验证：推迟到实际落地时进行（用户 2026-09-12 明确）；遵循 R13 多源数据对比验证原则。
- **转债行情/K 线**：东财限流 + 无备源（腾讯不覆盖、新浪旧接口 404）——P2 以来唯一长期未闭环的数据项。
